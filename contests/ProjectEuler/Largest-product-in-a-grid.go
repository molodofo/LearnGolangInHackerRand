...
func main() {
	...
	for i := 0; i < 20; i++ {
		for j := 0; j < 20-3; j++ {
			res = max(matrix[i][j]*matrix[i][j+1]*matrix[i][j+2]*matrix[i][j+3], res)
		}
	}
	for i := 0; i < 20-3; i++ {
		for j := 0; j < 20; j++ {
			res = max(matrix[i][j]*matrix[i+1][j]*matrix[i+2][j]*matrix[i+3][j], res)
		}
	}
	for i := 0; i < 20-3; i++ {
		for j := 0; j < 20-3; j++ {
			res = max(matrix[i][j]*matrix[i+1][j+1]*matrix[i+2][j+2]*matrix[i+3][j+3], res)
		}
	}
	for i := 0; i < 20-3; i++ {
		for j := 0; j < 20-3; j++ {
			res = max(matrix[i][j+3]*matrix[i+1][j+2]*matrix[i+2][j+1]*matrix[i+3][j], res)
		}
	}
	fmt.Println(res)
}
