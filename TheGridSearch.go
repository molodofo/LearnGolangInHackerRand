...
func GridSearch(r1, c1, r2, c2 int, mx1, mx2 [][]byte, answer *bool) {
	//fmt.Println(r1, c1, r2, c2)
	*answer = false
	for j := 0; j < (r1-r2+1) && !(*answer); j++ {
		for k := 0; k < (c1 - c2 + 1); k++ {
			*answer = true
			for l := 0; l < r2 && *answer; l++ {
				for m := 0; m < c2; m++ {
					if mx1[j+l][k+m] != mx2[l][m] {
						*answer = false
						break
					}
				}
			}
			if *answer { // all correct, then break for
				break
			}
		}
	}
...
func main() {
	...
	for i := 0; i < t; i++ {
		...
		go GridSearch(r1, c1, r2, c2, mx1, mx2, &answer[i])
	}
	for i := 0; i < t; i++ {
		<-ch1
	}
